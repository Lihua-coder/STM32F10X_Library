...
//-------------- 替换开始 8bit-CPHA1 --------------
        ...
        {
            gpio_toggle_level(soft_spi_obj->sck_pin);        // 先到达第二个边沿
            if(0x80 & write_data)
                gpio_high(soft_spi_obj->mosi_pin);
            else
                gpio_low(soft_spi_obj->mosi_pin);
            ...
            gpio_toggle_level(soft_spi_obj->sck_pin);        // 再拉回空闲
            write_data = write_data << 1;
            read_data  = read_data  << 1;
        }
//-------------- 替换结束 ------------------------
...
// 备注信息     
//-------------------------------------------------------------------------------------------------------------------
void soft_spi_init (soft_spi_info_struct *soft_spi_obj, uint8 mode, uint32 delay, gpio_pin_enum sck_pin, gpio_pin_enum mosi_pin, gpio_pin_enum miso_pin, gpio_pin_enum cs_pin)
{
    zf_assert(soft_spi_obj != NULL);
    ...
    if(0 == mode || 1 == mode)
    {
        gpio_init(sck_pin, GPO_PUSH_PULL, 0);                       // IO 初始化
    }
    else
    {
        gpio_init(sck_pin, GPO_PUSH_PULL, 1);                      // IO 初始化
    }
    gpio_init(mosi_pin, GPO_PUSH_PULL, 1);                         // IO 初始化
    ...
    else
    {
        ...
        gpio_init(soft_spi_obj->miso_pin, GPI_PULL_UP, 1);     // IO 初始化
    }
    ...
    else
    {
        ...
        gpio_init(soft_spi_obj->cs_pin, GPO_PUSH_PULL, 1);         // IO 初始化
    }
}
