...
static void ips200_write_data(uint8 dat)
{
    uint32 *odr1 = (uint32 *)IPS200_DATAPORT1;
    uint32 *odr2 = (uint32 *)IPS200_DATAPORT2;

    uint32 val1 = *odr1 & ~(0x0F << DATA_START_NUM1);
    uint32 val2 = *odr2 & ~(0x0F << DATA_START_NUM2);

    *odr1 = val1 | ((dat & 0x0F) << DATA_START_NUM1);
    *odr2 = val2 | ((dat >> 4) << DATA_START_NUM2);
}
...
void ips200_draw_line (uint16 x_start, uint16 y_start, uint16 x_end, uint16 y_end, const uint16 color)
{
    ...
    do
    {
        ...
        if(func_abs((int16)(y_start - y_end)) > func_abs((int16)(x_start - x_end)))
        {
            while(y_start != y_end)
            {
                ips200_draw_point(x_start, y_start, color);
...
#if IPS200_USE_SOFT_SPI
        soft_spi_init(&ips200_spi, (uint8)0, (uint32)IPS200_SOFT_SPI_DELAY, (gpio_pin_enum)IPS200_SCL_PIN, (gpio_pin_enum)IPS200_SDA_PIN, (gpio_pin_enum)SOFT_SPI_PIN_NULL, (gpio_pin_enum)SOFT_SPI_PIN_NULL);
#else
...
//        gpio_init(ips_bl_pin, GPO, GPIO_HIGH, GPO_PUSH_PULL);
        gpio_init(IPS200_DC_PIN_SPI, GPO_PUSH_PULL, 0);
        gpio_init(ips_rst_pin, GPO_PUSH_PULL, 0);
        gpio_init(ips_cs_pin,  GPO_PUSH_PULL, 1);
        gpio_init(ips_bl_pin,  GPO_PUSH_PULL, 1);   
		}
    else
    {
...
//        gpio_init(ips_cs_pin, GPO, GPIO_HIGH, GPO_PUSH_PULL);                   // LCD_CS
        gpio_init(ips_rst_pin,  GPO_PUSH_PULL, 0);                   // RTS
        gpio_init(ips_bl_pin,  GPO_PUSH_PULL, 0);                    // BL
        gpio_init(ips_cs_pin,  GPO_PUSH_PULL, 1);                   // LCD_CS
...
//        gpio_init(IPS200_RS_PIN_PARALLEL8, GPO, GPIO_HIGH, GPO_PUSH_PULL);      // LCD_RS
        gpio_init(IPS200_RD_PIN_PARALLEL8,  GPO_PUSH_PULL, 0);       // LCD_RD
        gpio_init(IPS200_WR_PIN_PARALLEL8,  GPO_PUSH_PULL, 0);       // LCD_WR
        gpio_init(IPS200_RS_PIN_PARALLEL8,  GPO_PUSH_PULL, 1);      // LCD_RS
...
//        gpio_init(IPS200_D1_PIN_PARALLEL8, GPO, GPIO_HIGH, GPO_PUSH_PULL);      // LCD_D1
        gpio_init(IPS200_D0_PIN_PARALLEL8,  GPO_PUSH_PULL, 1);      // LCD_D0
        gpio_init(IPS200_D1_PIN_PARALLEL8,  GPO_PUSH_PULL, 1);      // LCD_D1
...
//        gpio_init(IPS200_D3_PIN_PARALLEL8, GPO, GPIO_HIGH, GPO_PUSH_PULL);      // LCD_D3
        gpio_init(IPS200_D2_PIN_PARALLEL8,  GPO_PUSH_PULL, 1);      // LCD_D2
        gpio_init(IPS200_D3_PIN_PARALLEL8,  GPO_PUSH_PULL, 1);      // LCD_D3
...
//        gpio_init(IPS200_D7_PIN_PARALLEL8, GPO, GPIO_HIGH, GPO_PUSH_PULL);      // LCD_D7
        gpio_init(IPS200_D4_PIN_PARALLEL8,  GPO_PUSH_PULL, 1);      // LCD_D4
        gpio_init(IPS200_D5_PIN_PARALLEL8,  GPO_PUSH_PULL, 1);      // LCD_D5
        gpio_init(IPS200_D6_PIN_PARALLEL8,  GPO_PUSH_PULL, 1);      // LCD_D6
        gpio_init(IPS200_D7_PIN_PARALLEL8, GPO_PUSH_PULL, 1);      // LCD_D7
    }
